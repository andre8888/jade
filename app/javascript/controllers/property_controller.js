...
export default class extends Controller {
    ...
    async fetchPropertyData() {
        ...
        if (response.ok) {
            ...
            if (body.data.property.sale_price) {
                this.sale_priceTarget.value = body.data.property.sale_price
            }
            if (body.data.statistics.monthly_median_rent) {
                this.est_monthly_rent_ltrTarget.value = body.data.statistics.monthly_median_rent
            }
            if (body.data.statistics.avg_daily_rate) {
                this.avg_daily_rateTarget.value = body.data.statistics.avg_daily_rate
            }
            if (body.data.statistics.occupancy_rate) {
                this.occupancy_rateTarget.value = (body.data.statistics.occupancy_rate * 100).toFixed(2)
            }
            ...
            this.property_tax_rateTarget.value = body.data.property.property_tax_rate
            // if (body.data.property.property_tax_rate) {
            //     this.property_taxTarget.value = body.data.property.property_tax_rate
            // }
            this.monthly_hoaTarget.value = body.data.property.monthly_hoa
            if (body.data.property.monthly_insurance) {
                this.annual_insuranceTarget.value = body.data.property.monthly_insurance / 2 // estimate is too high!
            }
            if (body.data.property.year_built) {
                this.year_built = body.data.property.year_built
    ...
    showDailyRateDetails(body) {
        if (body.data.statistics.projected_revenue) {
            this.projected_revenueTarget.innerHTML = 'Projected Revenue: $' + body.data.statistics.projected_revenue
        }
    }

    showRentalDetails(body) {
        if (body.data.statistics.monthly_median_rent) {
            this.rent_medianTarget.innerHTML = 'Median: $' + body.data.statistics.monthly_median_rent
        }
        if (body.data.statistics.monthly_min_rent) {
            this.rent_minTarget.innerHTML = 'Min: $' + body.data.statistics.monthly_min_rent
        }
        if (body.data.statistics.monthly_max_rent) {
            this.rent_maxTarget.innerHTML = 'Max: $' + body.data.statistics.monthly_max_rent
        }
    }

    showPropertyDetails(body) {
        if (body.data.property.num_beds) {
            this.bedroomTarget.innerHTML = pluralize('bedroom', body.data.property.num_beds, true)
        }
        if (body.data.property.num_baths) {
            this.bathroomTarget.innerHTML = pluralize('bathroom', body.data.property.num_baths, true)
        }
        if (body.data.property.living_area) {
            this.living_areaTarget.innerHTML = pluralize('sq foot', body.data.property.living_area, true)
        }
        if (body.data.property.year_built) {
            this.year_builtTarget.innerHTML = 'Year Built: ' + body.data.property.year_built
        }
        if (body.data.property.zillow_url) {
            this.property_linkTarget.href = body.data.property.zillow_url
        }
        if (body.data.property.sale_estimate) {
            this.market_valueTarget.innerHTML = 'Market Value: $' + body.data.property.sale_estimate
        }
    ...
    handleMonthlyExpenses() {
        ...
        this.monthly_property_tax = ((parseFloat(this.property_tax_rateTarget.value || 0) / 100) * this.sale_price).toFixed(2) / 12
